-- Remove the game-specific ceiling while retaining positive whole numbers.
alter table public.profiles
  drop constraint profiles_current_number_check,
  add constraint profiles_current_number_check check (current_number >= 0);

alter table public.sightings
  drop constraint sightings_number_check,
  add constraint sightings_number_check check (number >= 1);
